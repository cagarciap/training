pipeline {
    ...
    parameters {
        ...
        string(name: 'URL_HOST', defaultValue: '', description: 'Url')
    ...
    stages{
        ...
        stage('Run container'){
            steps {	
                ...
                echo "${TIME}"
                echo "${ENDPOINT}"
                sh "docker run -d image-test python3.7 /usr/perfexp-tutorial/prueba.py ${URL_HOST} ${PORT} ${ENDPOINT} ${TEST} ${CONCURRENCY} ${RAMP_UP} ${STEPS} ${TIME}"
            }
        }
    } 
}